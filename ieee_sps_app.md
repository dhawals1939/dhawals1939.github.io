...
I am seeking SPS Scholarship funding because my research is experimental and hardware-intensive. I build computational imaging and sensing systems where signal processing is part of acquisition: the illumination, waveform, timing, physical modulation, sampling, and reconstruction are designed together. This is different from only applying signal processing after data has already been captured.

The funding would support hands-on system development. In acousto-optic scanning, we investigate whether ultrasound-driven refractive-index fields can replace mechanical line scanning. Making that work requires iterations with acoustic transducers, optics, water-tank geometry, waveform generation, triggering, calibration, and sound-light synchronization. In underwater optical communication, the same platform must support modulation, bit timing, sampling, and synchronization. In SDR-ToF LiDAR, access to software-defined radio hardware allowed us to test an RF/DSP path for high-speed optical sensing and sub-millimeter vibration recovery. Similar hardware iteration is also important for my current subsurface imaging work, where weak scattered optical signals must be measured carefully.

If permitted through Dartmouth, I would use the scholarship for hardware acquisition and experimental development. I would also use it to attend signal-processing-focused conferences, tutorials, and workshops such as ICASSP, ICIP, ICCP, and IEEE/CVF workshops. I would also support small outreach demonstrations that introduce students to sensing, optics, and signal processing.
